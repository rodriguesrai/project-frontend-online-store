...
const [product, setProduct] = useState<ProductInfoType>(initialProductDetails); 
const { id } = useParams(); 
useEffect(() => {
...
<p>{`Entrega grátis: ${product.shipping?.free_shipping ? 'Sim' : 'Não'}`}</p>
<p>{`Condição: ${product.condition === 'new' ? 'Produto novo.' : 'Produto usado.'}`}</p>
<p>{`Status do anúncio: ${product.status === 'active' ? 'Ativo' : 'Inativo'}`}</p> 
